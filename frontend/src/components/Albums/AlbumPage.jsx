...
const AlbumPage = ({handleLiked, setTrackId}) => {
    const {store} = useContext(Context)
    const params = useParams()
    const albumId = params.id

    useEffect(() => {
        store.getAlbumTracks(albumId)
    ...
    return (
        <Box>
            <Box sx={{height: 300, background: "linear-gradient(#565ce1, #7b7fe2)",
                ...
                <Box sx={{ml: 3}}>
                    <img className="robi" src={store.album.album_picture} alt="Album" />
                </Box>
                <Box sx={{ml: 4}}>
                    <Typography variant="h1" sx={{textTransform: "uppercase", fontWeight: 700, color: "#fff"}}>
                        {store.album.album_title}
                    </Typography>
                </Box>
            </Box>
            <Box
